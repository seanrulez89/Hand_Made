...
public class StrategyManager {
	...
	// 아군 방어 건물 건설 위치
	BuildOrderItem.SeedPositionStrategy seedPositionStrategyOfMyDefenseBuildingType; // 권순우 0617 현상태의 전략 거점이 어딘가를 판단하는것으로 공격 방어와 연계되어야 함
...
//		expansion ();
	...
	void setVariables(){
		...
		if (myRace == Race.Zerg) {
			...
			necessaryNumberOfCombatUnitType1 = 8;                      // 공격을 시작하기위해 필요한 최소한의 마린 유닛 숫자 
			...
			= BuildOrderItem.SeedPositionStrategy.SecondChokePoint;	// 앞마당
...
//					BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);	
	...
	/* 권순우 0617 다음의 코드는
	 * 내가 테란, 상대가 프로토스일때, 다크템플러에 털릴까봐 긴급히 터렛을 추가하는 코드이다.
	 * 5드론 저글링을 상대하기 위해 변형해서 사용할 수 있을 것으로 보임
...
/* 권순우 0617
 * 확장을 위해 만든 코드이다.
 * 작년에 결국 잘 안됐는데 다시 보니까 리스트 생성하는 부분하고 몇군데 고치면 바로 될 것 같은데?	
*/
 ...
 * 권순우 0617
 * 테란 레이스 은신 개발 관련코드이다.
 * 저그라서 필요없는데 혹시 어디 써먹을까 싶어서 남겨둔다.	
 ...
 */
	...
	/*
	 *  권순우 0617
	 *  방어 / 공격 / 앨리(elimination)
	 *  3 단계로 구분하였으며
	 *  방어모드에서는 공격여부를 판단하고 
	 *  공격모드에서는 방어여부나 앨리여부를 판단한다.
	 *  현재 상태에서 어떤 상태로 넘어갈지를 판단한다.
	 */
...
///////	
					...
					for(int i = 1 ; i < size ; i++) // 0이면 우리 기지에 짓더라 이게 무슨 순서가 있는건지... 0623 이거 좀 손봐라 기지만 계속 지어
					{
						if(MyBotModule.Broodwar.isExplored(startLocation.get(i).getTilePosition()))
						{
							...
							if (instance.hasBuildingAroundBaseLocation(startLocation.get(i), myPlayer, 200) == false)
							{
								...
								{
									BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Hatchery, startLocation.get(i).getTilePosition(),false);
									BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Zerg_Extractor, startLocation.get(i).getTilePosition(),false);
									//System.out.println(size);
									break;
								}
							}
	...
	boolean isTimeToStartElimination(){
		...
		if (myInstance.hasBuildingAroundBaseLocation(enemyMainBaseLocation, enemyPlayer, 200) == false && enemyKilledWorkerUnitCount >= 10)
		{
			return true;
		}
		*/
...
/*			if(countMyAttackUnitAround(position05,6) > 0.1 * countMyAttackUnitAll ())
			...
			*/
			...
			/* 권순우 0617 
			 * 노승호의 역작이다.
			 * 올해는 아래 코드를 변형해서 오버로드를 은신캐 근처로 이동시키는 코드를 짜면 되겠다.
			 * 
			 * 제일 좋은 것은 은신캐 근처에서 가장 가까운 오버로드를 이동시키는 것인데
			 * 적군 유닛의 위치를 중심으로 가장 가까운 아군 유닛중에서 오버로드면 와라!
			 * 
			 * 
			...
			*/
			...
			for (Unit unit : myAllCombatUnitList) {
				 ...
				 * 권순우 0617
				 * 지금 이 부분에 구현되는 코드가 
				 * 실제 전투시에 각 유닛이 수행할 행동을 의미한다
				 * 
				 * 작년에도 나쁘지는 않았는데
				 * 올해는 개별 유닛말고 부대단위 유닛으로 조정하면 좀 더 좋은 효과가 있겠다.
				 * 가령 일점사
...
//0623								commandUtil.attackMove(unit, targetPosition); // 0622 일단은 이렇게 하는데 뮤탈이 먼저 들어가서 일꾼 잡고 있었으면 상황이 살짝 달라진다 굳이 나올 필요 없는데 나오는 형국
//0623								MUTAL_MOVEMENT(unit, null, targetPosition);
							...
							else
							{
							 ...
							 /* 권순우 0617
							 * 가장 까다로운 부분이다.
							 * 메딕, 사이언스 베슬 등 특수 유닛은 일단 전투지역으로 이동을 시키고 뭔가를 해야되는데
							 * 이 부분을 해결하는 것이 매우 까다롭다.
							 * 
							 * 올해는 각 유닛별로 고유번호가 있다는 것을 알았으니
							 * 단순히 가까운 유닛을 찾는 것이 아니라 
							 * 전담 마크 개념을 구현해 볼 수 있을 것이다.
							 * 과연?*/
	...
	void executeBuildingConstruction() {
		...
		numberOfMyCombatUnitTrainingBuilding += ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Zerg_Hatchery, null);
		numberOfMyCombatUnitTrainingBuilding += ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Zerg_Lair, null);
		numberOfMyCombatUnitTrainingBuilding += ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Zerg_Hive, null);
		...
		if (isPossibleToConstructCombatUnitTrainingBuildingType == true
			...
			if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Zerg_Hatchery) == 0 ) 
			{
				...
				&& BuildManager.Instance().getAvailableMinerals() > 700 
	...
	void executeUpgradeAndTechResearch() {
		...
		if (myRace == Race.Zerg) {
			...
			if (myPlayer.completedUnitCount(UnitType.Zerg_Spire) > 0 && myPlayer.gatheredGas()>200 && myPlayer.completedUnitCount(UnitType.Zerg_Mutalisk) > 12) {
